...
// ** ------------------------------------------------------------------------------------------------------------------ ** // 
			...
			if (strtoupper($array[2]) == "CORREO") {
				...
				if ($cantidad != 0) {
					...
					if ($cantidad == 1)
						$datosEmpleados->Mensaje = 'Datos de ' . $cantidad . ' empleado, (desde direccion ip '.getRealIP().')';
					else
						$datosEmpleados->Mensaje = 'Datos de ' . $cantidad . ' empleados, (desde direccion ip '.getRealIP().')';
...
// ** ------------------------------------------------------------------------------------------------------------------ ** // 
				...
				$datosEmpleados = new stdClass();
				$datosEmpleados->Mensaje = 'Por favor revise los parámetros de consumo del servicio web, (desde direccion ip '.getRealIP().'). (Error 01).';
			}
        ...
        $datosEmpleados = new stdClass();
		$datosEmpleados->Mensaje = 'No tiene permiso para realizar esta petición, (desde direccion ip '.getRealIP().'). Póngase en contacto con el administrador del servicio web. (Error 02).';
    }
} else {
	// Mensaje si el token no corresponde o la opción de consumo es errada
	$datosEmpleados = new stdClass();
	$datosEmpleados->Mensaje = 'Por favor revise los parámetros de consumo del servicio web y que el token proporcionado sea el correcto, (desde direccion ip '.getRealIP().'). (Error 03)';
}
